...
It loads an **arm9loaderhax.bin** (always init the screens) ARM9 payload from the root of the SD card, CTRNAND, TWLN, TWLP, respectively, at address 0x23F00000.
...
There is a emergency mode, loading a payload (**emergency.bin**) from SD card to memory address 0x25F00000 if an exception is triggered, but if that fails due to another bug, it will try again, this time reading directly from the SD card (**emergency_regs.bin**).
